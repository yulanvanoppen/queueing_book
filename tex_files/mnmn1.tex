...
\begin{extra}[Multi-server queue with blocking]\label{ex:41}
...
\begin{exercise} Give an example of a system with a finite calling population.
  \begin{solution}
...
Observe that here we make a subtle implicit assumption; in Section~\ref{sec:poisson-arrivals-see} we elaborate on this assumption.
...
To relate these two `views', i.e., time-average versus observer-average, we need a new concept, \emph{PASTA}, to be developed in in Section~\ref{sec:poisson-arrivals-see}.
