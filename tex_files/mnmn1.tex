...
\begin{exercise}
  ...
  \begin{solution}
    $\lambda(n) = \lambda$ if $n< c$, and $\lambda(n)=0$ for $n\geq c$. Then, $\mu(n) = n \mu$ for $n\leq c$. (And $n$ can never be larger than $c$, since $\lambda(n) = 0$ for $n\geq c$.) Define $\rho = \lambda/c \mu$. $p(n) = G^{-1}(c\rho)^n/n!$, $G=\sum_{n=0}^{c} (c\rho)^n/n!$. $\E{L_Q}=0$. 
    \begin{equation*}
      \begin{split}
      \E{L_S} 
&= \sum_{n=0}^{c} n p(n) = G^{-1} \sum_{n=0}^c n (\lambda/\mu)^n/n! \\
& G^{-1} \lambda/\mu \sum_{n=0}^{c-1} (\lambda/\mu)^{n-1}/(n-1)! =\\
& \frac{\lambda}\mu \frac{G - (\lambda/\mu)^c/c!}{G}.
      \end{split}
    \end{equation*}
  \end{solution}
\end{exercise}
