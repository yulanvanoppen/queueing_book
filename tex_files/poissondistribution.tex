...
\hint{Realize that     $\P{N(t)=k} = \P{A_k \leq t} - \P{A_{k+1} \leq t}$.}
    \begin{solution}
...
\begin{question}(\tbd I added this to clarify the next question, you can skip it for year 16/17.)
...
\end{tikzpicture}
  ...
  \hint{Use a conditioning argument or use moment generating
    functions. In particular, for conditioning, use that
    $\P{AB} = \P{A\given B}\P{B}$. More generally, if the set $A$ can
    be split into disjoint sets $B_i$, i.e, $A=\bigcup_{i=1}^n B_i$,
    then
    \begin{equation*}
...
\frac{\lambda}{\lambda+\mu}.
    ...
    holds for any time $h$, whether it is small or not.  \hint{Suppose
      ...
      $\P{N_a(h) = 1, N_s(h) = 0 | N_a(h) + N_s(h) = 1}$ is the
      probability that $N_a(h)=1$ and $N_s(h)=0$ \emph{given} that
      $N(t)=1$. In other words, the question is find out that, given
      one of the two processes `fired', what is the probability that
      $N_a$ was the one that `fired'.}
