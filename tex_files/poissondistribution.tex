...
The Poisson \emph{process} $\{N(t), t\geq 0\}$ is a much more complicated
object than the Poisson distributed random variable $N(t)$. The
process is the set of random variables $N(t)$ for all $t\geq 0$, which is an \emph{ uncountable set} of random variables, whereas $N(t)$ is just \emph{one} random variable that is Poisson
...
To see that the exponential distribution can be derived from the Poisson distribution, observe that, if there are no failures in some interval $[0,t]$, then it
...
analysis of queueing process. Thus,  a counting process $\{N(t)\}$ is a \recall{Poisson process} with rate $\lambda$ if and only if  the inter-arrival times $\{X_i\}$ are i.i.d. and exponentially distributed with mean $1/\lambda$,  in short, 
\begin{equation*}
X_i\sim \exp(\lambda) \Leftrightarrow N(t) \sim P(\lambda t).
\end{equation*}

Define the  \recall{indicator function} $\1{A}$  of the set $A$ as $\1{A}=1$ if the event
$A$ is true, and $\1{A}=0$ if $A$ is not true. 
...
\begin{exercise} 
Show that   $N_\lambda(t) + N_\mu(t) \sim \text{P}((\lambda + \mu)t)$. 
  \begin{hint}
Use a conditioning argument or use moment generating
    functions 
