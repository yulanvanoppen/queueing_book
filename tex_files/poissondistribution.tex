...
\begin{hint}
  Recall that $M_{N(t)}'(0) = \E{N(t)}$ and $M_{N(t)}''(s)= \E{(N(t))^2}$. 
\end{hint}
\begin{solution}
...
Below, and elsewhere, we use the following efficient notation:  the function $f=o(h)$ means that $f$ is such $f(h)/h \to 0$ as $h\to 0$.  The next exercise helps you practice with this. 
\begin{exercise}
  If $c$ is a constant (in $\R$) and the functions $f, g =o(h)$, then $c\cdot f = o(h)$, $f+g=o(h)$, $f(h) \to 0$ as $h\to 0$, and $f\cdot g=o(h)$.  (If you believe this right away, you may skip the solution. The proof is perhaps harder than you think.) 
 \begin{solution}
First, 
\begin{align*}
...
\end{align*}
As a consequence, $f=o(h)\implies -f = o(h)$.  Therefore $|f| = \max\{f, -f\} = o(h)$. Furthermore, letting $f^+ = \max\{f, 0\}$, we have that $2f^+ = |f| + f$, hence $f^+=o(h)$. Likewise $f^- = f-f^+ = o(h)$. 
Then, if $h \in(0,1)$, 
\begin{equation*}
  -f^-(h)/h \leq f(h) \leq f^+(h)/h, 
\end{equation*}
we see that $f(h) \to 0$ since $f^-, f^+=o(h)$.  Finally,   $f\cdot g = o(h)$, since
    \begin{align*}
...
\begin{equation*}
\P{N(0,s] =1\given N(0,t]=1} = \frac s t.  
\end{equation*}
Thus, if you know that an arrival occured during $[0,t]$, the arrival is distributed
...
In the computations, use the standard binomial formula $(a+b)^n = \sum_{i=0}^b {n \choose i} a^{n-i} b^i$. 
  \end{hint}
    \begin{solution}
Choose $B_i = \{N_\lambda(t) = i\}$. Then, $\{N_\lambda(t) + N_\mu(t) = n\} = \cup_{i=0}^n \{N_\lambda(t) = i\}$. (To see this, consider an example. Cats arrive at rate $\lambda$, dogs with rate $\mu$. Suppose that $5$ animals (cats plus dogs) arrived. Then the number of cats must be in the set $\{0, 1, \ldots, 5\}$, and if I know that the number of cats is, e.g., 3, then the number of dogs is 2.)
...
What is the
  ...
  $\{N_\lambda(h) = 1, N_\mu(h) = 0\}\cap\{N_\lambda(h) + N_\mu(h) = 1\}$?
    \begin{solution}
There are two arrival processes, arrivals of the first type occur with rate $\lambda$, arrivals of the second  type with rate $\mu$.  If the event  $\{N_\lambda(h) + N_\mu(h) = 1\}$ is true, it tells us that precisely one arrival occured.  The event $\{N_\lambda(h) = 1, N_\mu(h) = 0\}$ means that up to time $t$, only an arrival of the first type occurred. Thus, the intersection of these events means that an arrival occured \emph{and} that this arrival was of the first type. 
    \end{solution}
\end{exercise}
\end{comment}

Again, there are two Poisson arrival processes. Let arrivals of the first type occur with rate $\lambda$, arrivals of the second  type with rate $\mu$.  Given that an arrival occured, the next question asks you to compute  the probablity that the arrival was of the first type. 
...
\frac{\lambda}{\lambda+\mu}.
    ...
    \begin{hint}
Use the standard formula for conditional probability and that  $N_\lambda(t) + N_\mu(t) \sim \text{P}((\lambda + \mu)t)$ to conclude
% Suppose
...
%       that $N(t)=1$. Use the standard formula for condtional probabiliy. 
    \end{hint}
    \begin{solution}
  With the above:
  \begin{equation*}
