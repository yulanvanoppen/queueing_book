...
\begin{exercise}
  Show that if $N(t)\sim P(\lambda t)$, $\E{N(t)} = \lambda t$ and $\V{N(t)} = \lambda t$. 
  \begin{solution} 
...
\begin{equation*}
\phi_n(z) = \E{z^{N(t)}} = \E{z^{N_a(t)}} \E{z^{N_s(t)}} = e^{\lambda(z-1)}e^{\mu(z-10} = e^{(\lambda +\mu)t (z-1)}. 
\end{equation*}
...
$\phi_a(z)$ but with $\lambda+\mu$ replacing $\lambda$, we can conclude that $N(t)\sim P((\lambda +\mu) t)$.
    \end{solution}
\end{exercise}
...
\begin{exercise}
 Assume that $N_a(t)\sim \text{P}(\lambda t)$ ,
  $N_s(t) \sim \text{P}(\mu t)$ and independent.  What is the
