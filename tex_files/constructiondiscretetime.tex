...
\begin{exercise}[\faPhoto\/ Queue with Blocking] a queueing system
...
\begin{exercise}[\faPhoto\/ Estimating the lead time distribution] 
...
\begin{exercise}[\faPhoto\/ Yield loss]
...
\begin{exercise}[\faPhoto\/ N-policies] A machine can switch on and off.
...
\begin{exercise}[\faPhoto\/ Queue length dependent server capacity]\label{ex:50}
...
\begin{exercise}[\faPhoto\/ Fair queueing] \label{ex:51}
...
\begin{exercise} [\faPhoto\/ Priority queueing] Another interesting situation is
...
\begin{exercise} [\faPhoto\/ Queues with reserved service capacity] Consider a single-server that
...
\begin{exercise} [\faPhoto\/ Queue with protected service capacity and lost capacity] Consider a
...
\begin{exercise}[\faPhoto\/ Tandem  networks]
...
\begin{exercise}[\faPhoto\/ A tandem queue with blocking]
...
\begin{exercise}[\faPhoto\/ Merging departure streams]
...
\begin{exercise}[\faPhoto \/ Merging incoming streams]
...
L_k &= L_{k-1}+a_k^a + a_k^b - d_k.
    ...
    start of the period in queue, i.e., $L_{k-1}-d_k$, are cleared;
    ...
    $W_{k,-} = \lfloor L_{k-1}/c\rfloor$.  Similarly, the maximal
    waiting time is
    $W_{k,+} = \lfloor (L_{k-1}+a_k^a + a_k^b) /c\rfloor$.
...
\begin{exercise}[\faPhoto\/ Splitting streams]
...
\begin{exercise}[\faPhoto\/ Inventory control] The recursions used in the
