...
\begin{question}(Inventory control) The approach demonstrated in the
  ...
  $M_k$ items per week during normal working hours, and maximally $N_k$
  items during extra (weekend and evening hours). Let, for period $k$,
  \begin{align*}
    S_k &= \text{Sales, i.e., number of items sold, in  period $k$}, \\
    p_k &= \text{Profit per item sold in  period $k$}, \\
    D_k &= \text{Demand in  period $k$}, \\
    X_k &= \text{Production capacity during normal hours}, \\
    Y_k &= \text{Production capacity during extra  hours}, \\
    c_k &= \text{Production cost per unit during normal  hours}, \\
    d_k &= \text{Production cost per unit during extra  hours}, \\
    h_k &= \text{holding cost per  unit at the end of the period}, \\
    I_k &= \text{On hand inventory level at the end of the period}. \\
  \end{align*}
  Management needs a production plan for the next $T$ periods
  specifying the number of items to be produced for the next
  periods. Formulate this problem as an optimization problem, and take
  into account the inventory dynamics.
  \begin{solution}
    The decision variables are $X_k$, $Y_K$ and $S_k$ (note, it is not
    necessary to meet all demand. As the production cost and profit
    may vary per period.)  The objective is 
    \begin{equation*}
      \max \sum_{k=1}^T (p_kS_k -c_k X_k - d_k Y_k - h_k I_k).
    \end{equation*}
The constraints are 
\begin{align*}
  0&\leq S_k \leq D_k, \\
...
\begin{question} (Estimating the lead time distribution.)  Take
  ...
  FIFO sequence.
  \begin{enumerate}
  \item Find an expression for the earliest possible departure time
    $l_-(k)$ of a job that arrives at time $k$.
  \item Find an expression for the latest possible departure time
    $l_+(k)$ of a job that arrives at time $k$.
  \item How can you use $l_-$ and $l_+$ to  bound the waiting times?
  \end{enumerate}
    \begin{solution}
      \begin{enumerate}
      \item     A job that arrives at time $k$ cannot be served before period
    \begin{equation*}
    l_-(k):= \max\left\{l: \sum_{i=n}^l d_i < Q_{k-1}\right\}.
    \end{equation*}
  \item  It will be served before period
    \begin{equation*}
      l_+(k):= \min\left\{l: \sum_{i=n+1}^l d_i \geq
        Q_{k}\right\}.
    \end{equation*}
  \item Thus, the waiting time of jobs arriving in period $k$ must lie
    in $[l_-(k), l_+(k)]$.
      \end{enumerate}
  \end{solution}
\end{question}
...
\includegraphics{md1_sim} 

I multiply the mean of \texttt{(Q>20)} by 100 to get a percentage.
...
\includegraphics{mm1_sim} 

You should compare the scales of these two graphs. The second queue
behaves more wildly, as expected.

You can try to implement the other cases below and analyze them in the
same way. Hopefully you understand from the above that once you have
the recursions to construct/simulate the queueing process, you are `in
business'. The rest is easy, just make plots, do some counting, and so on.
  \end{solution}
\end{question}
