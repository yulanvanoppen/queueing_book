...
the system. In this section we will derive the famous \recall{Poisson arrivals see time averages}(\recall{PASTA}) condition  that ensures that  $\pi(n)=p(n)$ if the arrivals follow a Poisson process. 


We can make some progress by rewriting $\pi(n)$ in the following way.
Since $A(t)\to \infty$ as $t\to\infty$, it is reasonable that\footnote{See below for the proof.}
...
\begin{exercise}[\faFlask]\label{ex:8} Show that for the case of Exercises~\ref{ex:111} and~\ref{ex:112} $\pi(0)=1$ and $\pi(n)=0$, for $n>0$.
\begin{solution}
...
As the exercises above show, this property is not satisfied in general.
...
Thus, when customers arrive in accordance to a Poisson process (hence the inter-arrival times are exponentially distributed), it must be that $\pi(n) = p(n)$, and for the $M/M/1$ queue,
\begin{equation*}
...
\begin{exercise}[\faFlask, tricky]\label{ex:26}
...
resolution lies in the renewal reward theorem.
...
\begin{exercise}[\faFlask]\label{ex:dep}
...
\begin{exercise}[\faFlask]\label{ex:28}
...
\begin{exercise}[\faCalculator]\label{ex:17}
...
\begin{exercise}[\faFlask]\label{ex:burke}
