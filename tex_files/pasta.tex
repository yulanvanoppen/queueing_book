...
\begin{theorem}[Renewal Reward Theorem, $Y=\lambda X$]
  ...
  $N(t) = \max\{k : T_k \leq t\}$. Then $Y(t)/t$ has a limit iff
  $n^{-1}\sum_k^n X_k$ has a limit in which case $Y=\lambda X$, i.e.,
  \begin{equation*}
  \lim_{t \to \infty} \frac{Y(t)}t=Y \iff \lim_{n \to \infty} \frac 1n\sum_{k=1}^n X_k =X \implies Y=\lambda X.
  \end{equation*}
\end{theorem}
...
\begin{equation*}
    f_D(t) = f_{X+S}(t) \P{I} + f_S(t) \P B = (1-\rho) f_{X+S}(t) +
    \rho \mu e^{-\mu t}.
\end{equation*}
