...
the system. In this section we will derive the famous \recall{PASTA (Poisson arrivals see time averages} condition  that ensures that  $\pi(n)=p(n)$. 
...
\begin{exercise}\label{ex:8}
  ...
  $S_i=59$ minutes.  What are $\rho$, $p(0)$, $p(n)$ for $n\geq 2$. What is $\pi(n)$?
  What is the time average system length $\E L$? What is the queue
  length as observed by customers?  Check Eq.~(\ref{eq:13}) for this case.
\begin{hint}
...
\begin{exercise}
  Check that this relation holds for the system of Exercise~\ref{ex:8}
  \begin{solution}
 We have to check that $\lambda(n) p(n) = \lambda \pi(n)$.  First
...
At the exercises above show, this property is not satisfied for any general queueing
system.  However, it is true when the arrival process is
Poisson. This fact is typically called \recall{PASTA: Poisson
Arrivals See Time Averages}. Thus, when customers arrive in accordance to a Poisson process (so
...
Show that 
\begin{equation*}
\lambda  \pi(n) = \lambda(n) p(n) = \mu(n+1) p(n+1) = \gamma \delta(n).
\end{equation*}
...
\begin{align*}
  ...
  \frac{D(n,t)}{t} &=   \frac{D(n,t)}{D(t)} \frac{D(t)}{t} \to \delta(n)\gamma \\
\end{align*}
\end{solution}
\end{exercise}
...
\begin{exercise}
Show that~(\ref{eq:132}) is valid.
\begin{hint}
Check that the conditions of the renewal reward theorem are satisfied in the above prooff~\eqref{eq:1333}. Then define  
\begin{align*}
