...
\begin{equation*}
  Y_i(t) = \int_0^{D_{D(t)}} \1{L_S(s)=i} \d s
\end{equation*}
is the total time up to time $D(t)$ that there are $i$ items at the server. 

\begin{exercise}
Let $\tilde A_k$ be the moment the $k$th batch moves to the server and $D_k$ its departure time. Show that, when $S_{k,i}$ is the service time of the $i$th item of this batch,
\begin{equation*}
...
\end{equation*}
where  the limits exist by the strong law of large numbers. With  Exercise~\ref{ex:ER} 
the expected remaining number of items to be served of the batch in service can be simplified to
