...
$\E S$. Therefore the expected time the job spends in queue is
\begin{equation*}
  \E{W_{Q,b}} = \E{L} \E S;
\end{equation*}
...
the total time the server is busy. Thus, the probability that an
arbitrary arrival sees $i$ remaining items in service is
$\sum_{k=1}^n \1{B_k \geq i}/\sum_{k=1}^n B_k$, where we use the fact
that Poisson arrivals are uniformly distributed, conditional on a
given number of arrivals in an interval, to conclude that any item of
the $\sum_{k} B_k$ items is `chosen' with the same probability.
