...
Show that the requirement $\rho < 1$ leads to the following constraint on the minimal batch size~$B$ 
 ...
 B>\frac{\lambda \E R}{1-\lambda \E{S_0}}.
  \end{equation*}
  \begin{solution}
  From the expression for load ($\rho = \lambda \left(\E{S_0} + \frac{\E{R}}B\right)$) we get that $\lambda(\E{S_0} + \frac{\E{R}}B\right) < 1$, which is equivalent to $\frac{\E{R}}B\right) < \frac{\E{R}}B\right) < frac{1}\lambda\right - \E{S_0}$, which can be writen as $\frac{1}B\right) < \frac{\lambda\E{R}}1-\lambda\E{S_0}\right)$, finaly the result is obtained by taking the inverse.
  \end{solution}
\end{exercise}
