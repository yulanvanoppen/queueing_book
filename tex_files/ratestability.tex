...
\hint{Use the analogy with Eq.~\eqref{eq:27}.}
  \end{hint}
\begin{solution}
  \begin{equation*}
...
Define $\tilde X_k = S_{k-1}-X_k$.
 ...
 \begin{hint}
\hint{Remember that $\{X_k\}$ and $\{S_k\}$ are sequences of i.i.d. random variables. What are the implications for the expectations?}
...
\begin{question}
  ...
  \begin{hint}
\hint{What is the rate in, and what is the service capacity?}
  \end{hint}
  \begin{solution}
...
\begin{question}
  ...
  \hint{As a start, the function $\sin(t)$ has not a limit as
...
\begin{question}
  ...
  is right-continuous. 
    ...
    \hint{ Use that $A_{A(t)} \leq t < A_{A(t)+1}$. Divide by $A(t)$
