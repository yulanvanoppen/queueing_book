...
downcrossing in this section to derive the \emph{stationary}---also known as \emph{long-run time average} or \emph{steady-state},
...
\begin{exercise}[\faBalanceScale]\label{ex:36}
...
\begin{exercise}[\faBalanceScale]\label{ex:38}
...
be the fraction of time that $L(s) =n$ in $[0,t]$. Figure~\ref{fig:Y_1_t} illustrates the relation between $Y(n,t)$ and $A(n,t)$.
 ...
 \begin{exercise}[\faPhoto] \label{ex:111} Consider the following (silly) queueing process.
...
\begin{exercise}[\faPhoto,  Continuation of~\ref{ex:111}] \label{ex:112} 
...
\begin{exercise}[\faFlask]\label{ex:39}
...
\begin{exercise}[\faCalculator]\label{ex:4}
...
\begin{exercise}[\faPhoto] 
  Use the graph of the previous question and a level crossing argument to express the steady-state probabilities $p(n), n=0,\ldots, 3$ in terms of $\lambda$ and $\mu$.
  \begin{hint}
