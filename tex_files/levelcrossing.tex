...
The system \emph{crosses} level $n$ at time $t$ when its state changes from $n$ to $n+1$, either `from below' due to an arrival, or `from above' due to a departure, cf.
Figure~\ref{fig:A_n_t}.
...
To establish the section's main result Eq.~\eqref{eq:12} we need a few definitions that are quite subtle and might seem a bit abstract, but below we will provide intuitive interpretations in terms of system KPIs.
Once we have the proper definitions, the above result will follow straightaway.
Figure~\ref{fig:summaries} at the end of the section summarizes all concepts we develop here.
...
\end{equation}
\end{subequations}
Combining this with~\eqref{eq:15} we arrive at the \recall{level crossing equations}
...
In the next few sections we will make suitable choices for $\lambda(n)$ and $\mu(n)$ to model many different queueing situations so that, based on \eqref{eq:12}, we can obtain simple expressions for $p(n)$ in terms of the arrival and service rates.
...
\begin{exercise}
  Derive  $\E L = \sum_{n=0}^\infty n p(n)$ from~\eqref{eq:46}. 
\begin{solution}
...
\begin{exercise}
  Use the graph of exercise~\ref{ex:55} and a level crossing argument to express the steady-state probabilities $p(n), n=0,\ldots, 3$ in terms of $\lambda$ and $\mu$.
  \begin{hint}
...
always be used as we do here. The reason is that sometimes there does not exist a line between two states such that the state space splits into two disjoint
...
Then, $A(20,t)$ as defined in Eq.~(\ref{eq:19}) is the number of\emph{ blocked parcels} up to time $t$, and $A(20,t)/A(t)$ is the fraction of rejected parcels.
