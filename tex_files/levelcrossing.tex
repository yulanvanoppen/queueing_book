...
at time $t$ is equal to $A(t) - D(t)$, see Figure~\ref{fig:box}. Let
us denote an arrival as an `up-crossing' and a departure as a
...
$L(t)$ remains finite, or more generally $\lim_t L(t)/t = 0$, then it
must be that
\begin{equation*}
...
downcrossing in this section to derive the \recall{stationary}, also
know as \emph{long-run time average} or \emph{steady-state},
...
\caption{What goes in the box (i.e., $A(t)$) and has not yet left
...
occurs.  The main result we derive in this section is the rate at
which level $n$ is crossed from below and above must match, i.e.,
\begin{equation*}
...
\begin{align}
  A(n,t) &= \sum_{k=1}^\infty \1{A_k \leq t}\1{L(A_k-) = n} \label{eq:19} \\
\intertext{as the number of arrivals up to time
$t$ that saw $n$ customers in the system at their arrival, c.f. Figure~\ref{fig:Y_1_t},}
   Y(n,t) &= \int_0^t  \1{L(s) = n} \d s \label{eq:17} \\
\intertext{as the total time the system contains n jobs during $[0,t]$, and}
   p(n,t) &= \frac 1 t \int_0^t  \1{L(s) = n} \d s = \frac{Y(n,t)}t, \label{eq:18}
\end{align}
\end{subequations}
as the fraction of time that $L(s) =n$ in $[0,t]$ (Recall that $L(t)$
is \emph{right-continuous} in the definition of $A(n,t)$.)
...
\begin{equation*}
  \lambda(n) = \lim_{t\to\infty} \frac{A(n,t)}{Y(n,t)}, \quad p(n) =\lim_{t\to\infty} p(n,t),
\end{equation*}
...
see $n$ jobs in the system, while $Y(n,t)$ tracks the amount of time
the system contains $n$ jobs. Suppose that at $T$ a job arrives that
sees $n$ in the system. Then $A(n,T)=A(n, T-)+1$, and this job arrives
in an interval that it tracked by $Y(n,t)$, precisely because this job
sees $n$ in the system just prior to its arrival. Thus, just as
$A(t)/t$ is the total number of arrivals during $[0,t]$ divided by
$t$, $A(n,t)/Y(n,t)$ is the number of arrivals that see $n$ divided by
...
\begin{solution}
       Observe that
      $\1{A_k\leq t}\1{L(A_k-) = n} =\1{A_k\leq t}\1{L(A_k-) = n}
      \leq \1{A_k\leq t}$;
...
\begin{question}
  ...
  $\lambda > \gamma$, i.e., if the system is not rate-stable?
  \begin{solution}
...
Is it always true that 
 ...
 When $\gamma < \lambda$, $\lambda$ can still be finite, but
    the server is simply too slow. In that case both limits can exists,
    the first limit is positive but the second is $0$.

    Note that $\gamma\leq \lambda$ (always): jobs cannot depart faster
    than they arrive. Hence, we do not have to consider the case
    $\gamma>\lambda$ in the above.
  \end{solution}
\end{question}
...
\begin{question}
  ...
  \hint{For the acronym, observe that the service times and
...
\begin{question}
  ...
  At most 3 jobs fit in the system. Make a drawing

  Make a graph of the state-space and show, with arrows, the
...
\hint{First balance the rates accross the levels. Then solve for $\pi(k)$.}
  \begin{solution}
With level crossing
  \begin{align*}
    \lambda p(0)  &= \mu p(2) \\
