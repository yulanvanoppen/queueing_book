...
downcrossing in this section to derive the \recall{stationary}, also know as
...
\caption{What goes in the box, $A(t)$, and has not yet left, $D(t)$,
  must be in the box, hence $L(t)=A(t)-D(t)$. }
  \label{fig:box}
\end{figure}
...
\end{equation*}
as the \emph{arrival rate in state $n$.} Similarly, denoting by
\begin{equation*}
    D(n,t) = \sum_{k} \1{D_k \leq t} \1{L(D_k) = n}
  \end{equation*}
  the number of departures up to time $t$ that leave $n$ customers
  behind, we have that
\begin{equation*}
  \mu(n+1) = \lim_{t\to\infty} \frac{D(n,t)}{Y(n+1,t)},
\end{equation*}
is \emph{ the departure rate from state $n+1$}. (It is easy to get
