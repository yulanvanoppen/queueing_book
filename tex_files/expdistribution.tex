...
\begin{hint}
  What is the interpretation of the function $1-e^{-\lambda t}$?
\end{hint}
\begin{solution}
  If $\lambda<0$, then $1-e^{-\lambda t}$ grows to $-\infty$ if $t\to \infty$. Just by itsel this is not a problem. However, in our case $1-e^{-\lambda t}$ has the interpretation of a distribution function. Now, recall that a distribution function is bounded to values in the interval $[0,1]$.
...
The  above exercises can also be easily solved with the concept of \recall{moment generating function} which is defined for a random variable $X\geq 0$ and $t$ a real number as 
\begin{equation*}
...
\begin{exercise}
    ...
    \begin{align*}
      \E{X} &= M_X'(0)=\frac1\lambda, & 
      \E{X^2} &= M_X''(0)=\frac2{\lambda^2}, 
    \end{align*}
where $M'(t) = \frac{\d}{\d t} M(t)$. 
\begin{solution}
  $M_X'(0)=\lambda/(\lambda-t)^2$. Hence, $M_X'(0)=1/\lambda$. And, $M_X''(t)=2\lambda/(\lambda-t)^3$, hence $\E{X^2}=M_X''(1)=2\lambda/\lambda^3=2\lambda^{-2}$. 
\end{solution}
  \end{exercise}
...
\begin{exercise}
  Prove that when $X$ is exponentially distributed, $C^2 =1$.  
\begin{solution}
...
However, the idea of the exercise is to practice with the formulas. 
...
For the next step, we need to concept or  concept of conditional probability. To help you recall this, solve the next question.
...
When $X$ and $S$ are not exponentially distributed, such computations are much harder.   
\begin{exercise}
...
\begin{exercise}\label{ex:6}
 Use indicator functions to prove that $ \sum_{n=0}^\infty G(n) = \E B$.
    \begin{hint}
Write 
...
Let $S$ be a continuous random variable $S$ with distribution function $F$.
