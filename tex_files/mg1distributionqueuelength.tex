...
\end{equation*}
Noting that $\pi(n) = \delta(n)$, which follows from~\eqref{eq:39} and the fact that the $M/G/1$ queue length process has one-step transitions, we arrive at
\begin{equation}\label{eq:72}
  f(0) \pi(n+1) = \pi(0) G(n) + \sum_{m=1}^{n} \pi(m) G(n+1-m).
\end{equation}

Clearly, we have again obtained a recursion by which we can compute, iteratively, the state probabilities, and follow the approach sketched below Exercise~\ref{ex:13}.
...
\begin{exercise}[\faCalculator]
  ...
  Check that the queue length distribution of the $M/M/1$ queue satisfies~\eqref{eq:72}.
  \begin{hint}
    ...
    Then, with Eq.~\eqref{eq:41}, $f(n) = \alpha^n(1-\alpha)$ and $G(n) = \alpha^{n+1}$.
  ...
  \end{hint}
    \begin{solution}
Use the shorthands of the hint. We work in steps. First we check whether $\pi(0)=1-\rho$ of the $M/M/1$ queue satisfies~\eqref{eq:72}, then we check $\pi(1)$, and finally we check the general case with $n\geq 1$. 
...
Finally, we fill  $\pi(n) = \rho^n \pi(0)$ of the $M/M/1$ queue for $n\geq 1$.
For ease,  we divide both sides by $\pi(0)$ first.  We are left with checking that
\begin{align*}
    (1-\alpha)\rho^{n+1} 
