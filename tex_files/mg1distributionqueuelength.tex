...
\begin{equation*}
\1{L(D_{k-1})=n+1}\1{Y_k = 0} = \1{L(D_k) = n}.
\end{equation*}
With this result, we see that the number of downcrossings up to time
$t$ is such that
\begin{equation*}
  \begin{split}
  D(n+1, 0, t) 
...
&= \sum_{k=1}^\infty \1{D_{k}\leq t}\1{L(D_{k-1})=n+1}\1{Y_k=0}.
  \end{split}
\end{equation*}
Then, by using the definitions and limits developed in
...
\begin{equation*}
  \1{L(D_{k-1})=m}\1{Y_k > n-m+1} = \1{L(D_k) > n},
\end{equation*}
while if $m=0$ (think about this),
\begin{equation*}
  \1{L(D_{k-1})=0}\1{ Y_k > n} = \1{L(D_k) > n}.
\end{equation*}
Similar to our earlier work, by defining proper counting functions,
c.f., Exercise~\ref{q:555}, and taking suitable limits, we find 
that the upcrossing rate is
