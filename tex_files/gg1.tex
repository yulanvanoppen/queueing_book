...
This formula is reasonably accurate; for more, related, expressions we
...
\begin{question}
  Show that the approximation~\eqref{eq:7} for the $G/G/1$ queue
  reduces to the result known for the $M/M/1$ and $M/G/1$ queues.
  \hint{ Take $c=1$.}
  \begin{solution}
...
\begin{question}
  ...
  \hint{ Let $S_n = \sum_{k=1}^n X_k$ and $\sigma_n^2 = n\V X$. Then,
    under conditions you can find on the internet,
    \begin{equation*}
    \frac{S_n - n \E X}{\sigma_n} \to N(0,1), \quad\text{as } n\to \infty,
    \end{equation*}
    where $N(0,1)$ is a normally distributed random variable with
    $\mu=0$ and $\sigma=1$. But then $S_n$ is approximately
    distributed as a normal r.v. $N(n\E X, \sigma_n)$ (since
    $N(n\E X, \sigma_n) = \sigma_n N(0,1) + n \E X$.)}
...
\begin{solution} If the number in the system is $n$, then the total
  ...
  \begin{equation*}
    W_{Q, k} \sim \sqrt{n \V S} N(n\E S, \sigma_n)
  \end{equation*}
...
\begin{question}
  ...
  \end{enumerate}
  \hint{ It took me some time to understand the details of part a. I
