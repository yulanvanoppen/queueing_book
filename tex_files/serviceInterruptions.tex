...
Up to now we assumed that servers are never interrupted while serving a job. In many situations this is not true, a person  might be receive short phone calls while working on a job, a machine mail fail in the midst of processing. In this section we develop a model to account for such interruptions, and compute the influence on the mean waiting time. 

Let us assume that a job's normal service, without interruptions, is given by $S_0$. The durations of the interruptions are given by the i.i.d. random variables $\{D_i\}$ and have common mean $\E D$ and variance $\V D$. If $N$ interruptions occur, the real, total, service time will then be
\begin{equation*}
...
It is important to realize that $\rho = \lambda \E S = \lambda \E{S_0}/A$, hence the load increases due to failures. 
...
\begin{exercise}
  ...
  \begin{solution}
    Because $S_0$ and $N$ are not independent.
  \end{solution}
\end{exercise}
...
\begin{exercise}
  ...
  \begin{hint}
    Use the results of Section~\ref{sec:serv-interr}.
  \end{hint}
\end{exercise}
\begin{exercise}
...
EW + ES # sojourn time
\end{pyconsole}
The average time in queue reduces for $\approx 0.6$ to $\approx 0.2$ hours, a reduction by a factor 3. 
\end{solution}
\end{exercise}

\begin{comment}
\begin{exercise}
If $N=n$, show that $\E{S_n^2}=n\E{D^2} + n(n-1)(\E D)^2$.
\begin{hint}
  Use that $\left(\sum_{i=1}^n D_i\right)^2 = \sum_i D_i^2 + \sum_i \sum_{j\neq i } D_i D_j$, and that $D_i$ and $D_j$ are assumed to be independent.
\end{hint}
\begin{solution}
\begin{equation*}
  \begin{split}
...
\begin{exercise}
Now $N$ is a random variable, with $p_n=\P{N=n}$. Use this to show that
\begin{equation*}
%\E{S_N^2}=\sum_{n=0}^\infty p_n (n\E{D^2} + n(n-1)(\E D)^2) = \E N \V{D} + \E{N^2} (\E D)^2.
\E{S_N^2}= \E N \V{D} + \E{N^2} (\E D)^2.
\end{equation*}
\begin{solution}
\begin{equation*}
  \begin{split}
...
\begin{exercise}
Use the above to conclude that $\V{S_N} = \E{N} \V D + \V N (\E D)^2$.
\begin{solution}
\begin{equation}
  \begin{split}
  \V{S_N} 
...
\begin{exercise}
  What is the variance of the compound Poisson process?
\end{exercise}
