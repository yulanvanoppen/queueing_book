...
\nvf{ explain intuition behind the approximation. Basically, the
integrals over the service distribution of the $M/G/1$ can be hard to
carry out, so that we need to resort to numerical approaches. But
discretizing the service distribution comes down to approximating the
service times by batch jobs.}
...
that, in some sense, $X^{(n)}/n \stackrel{d}{\to} S$. \nvf{Explain,
  intuitively, convergence in distribution.}
...
\begin{question}
  ...
  $\E{L^{(n)}}/n \to \E W$? 
  \hint{What is the expected service time of one
