...
As such outages do not interrupt the processing of a job in service, we call such outages \recall{non-preemptive}.
...
sequence of random variables distributed as the common random variable ~$T$ and independent of $S_0$.
...
\begin{exercise}[\faCalculator]
  Show that the average effective processing time satisfies $ \E{S} = \E{S_0} + \E T/B$. Conclude that the effective server load including down-times is $\rho = \lambda \E{S}$. 
\begin{hint}
...
\begin{exercise}[\faCalculator]
  ...
  \begin{align*}
    \E{S^2} 
&= (1-p) \E{S_0^2} + p \E{(S_0+T)^2}\\
&= (1-p) \E{S_0^2} + p \E{S_0^2}  + 2 p \E{S_0} \E T + p \E{T^2}. 
  \end{align*}
