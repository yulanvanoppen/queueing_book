...
Consider two single-server stations in tandem. The service process at both stations
...
Which one is the better one to spend it on?  As stated, this problem is of course not clear cut. To come with a well balanced answer we need some extra information, such as which of the two
stations has the larger variability. However, with the queueing tools we developed up to now, we can provide some general insight into this problem. Realize the relevancy of modeling for this problem. Due to the lack  of money you can only achieve one of the alternatives; hence there   is no way of finding out what would have happened if you would have   made the other choice. Thus, we need models, simulations and/or experiments to find out what is the best option.

To obtain some initial insights, we first compute the expected waiting time in the queue at both
...
second or first station.  As a reference scenario, suppose that the arrival process has a Poisson distribution with rate $\lambda$, and that the service distributions at the first and second server,
respectively, are exponential with rate $\mu_1$ and $\mu_2$,
respectively. This is, from an analytic point of view, about the
simplest situation we can consider.  

\begin{exercise}
Show that for the reference situation the average queueing time is given by
\begin{equation}
\E{W_Q}  = \frac{\rho_1}{1-\rho_1} \frac1{\mu_1} + \frac{\rho_2}{1-\rho_2} \frac1{\mu_2},
\end{equation}
where $\rho_i = \lambda/\mu_i$ and $\E{S_i} = 1/\mu_i$, for $i=1,2$. 
\begin{hint}
Focus on the waiting times for each station separately, and realize that each is an $M/M/1$ queue.
\end{hint}
\begin{solution}
...
\begin{exercise}
Suppose we spend all our  money on the second server and that this suffices to entirely remove all  variability of  service process. This  is of course (a bit) optimistic; the situation will not get any  better than this. Show that  in this case
\begin{equation*}
  \E{W_Q}= \frac{\rho_1}{1-\rho_1} \frac1{\mu_1} +
  \frac12\frac{\rho_2}{1-\rho_2} \frac1{\mu_2}. 
\end{equation*}
is the total time in queue.
\begin{hint}
  If we can reduce all service variability at the second server, the second station can be modelled as an $M/D/1$ queue.
\end{hint}
\begin{solution}
...
of the second server.  This yields that the coefficient of variation
...
\begin{exercise}
Suppose now that we spend all our money on the first station, and that now the service process of the first station becomes completely regular. Motivate that 
\begin{equation*}
...
\begin{exercise}
  ...
  work station?
  \begin{solution}
    If it would be cheap. However, as the previous exercise explains, by reducing variability at the first station the waiting     time of all stations along the line goes down. Thus, by improving
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=1.8cm,
  ...
  \node[state] (6) [right of=5] {$p(\cdot, \cdot)$};
