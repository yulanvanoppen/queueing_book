...
Finding general conditions when to stop is not directly easy, but a pragmatic approach is too simple: stop at some (large) number $N$ such that $\pi(k)\ll \pi(0)$ for $k>N$ and steadily decreasing.\footnote{An interesting question, why should it decrease monotonically after some, large, $N$?}
