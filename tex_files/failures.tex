...
\begin{hint}
  Use that $\{D_i\}$ is a sequence of i.i.d. rv.s. Hence, $\E{\sum_{i=1}^n D_i} = n \E D$. 
\end{hint}
\begin{solution}
Since $\E{S_n} = n \E D$, 
\begin{align*}
...
To simplify this, we  assume at first that $S_0$ is known, so that $N\sim \text{Poisson}(\lambda_f S_0)$, hence $\E{N\given S_0} = \lambda_f S_0$ and $\E{N^2\given S_0}= \lambda_f S_0 + \lambda_f^2 S_0^2$. 
