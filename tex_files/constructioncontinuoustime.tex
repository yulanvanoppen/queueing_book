...
\begin{exercise}
  ...
  previous exercise.
    \begin{solution}
      You can check the files on \pyv{github} in the \pyv{progs}
      directory.
\begin{itemize}
\item \pyv{mm1.py} is the python implementation
\item \pyv{mm1.R} is the R implementation
\item \pyv{mm1.jl} is the julia implementation
\end{itemize}
...
\begin{solution}
 $L_Q(t) = A(t) - \tilde A(t)$. $L_S(t) = \tilde A(t) - D(t)$. This is in
    line with the fact that $L(t) = L_Q(t) + L_s(t) = A(t) - D(t)$. \\this is the same answer 
\end{solution}
\end{exercise}
...
\begin{solution}
 $L_Q(t) = A(t) - \tilde A(t)$. $L_S(t) = \tilde A(t) - D(t)$. This is in
    line with the fact that $L(t) = L_Q(t) + L_s(t) = A(t) - D(t)$. \\this is the same answer
\end{solution}
\end{exercise} \\I would actually leave this question out since it is already stated exactly above the question.
...
\begin{exercise} Show that $L(t) = A(t)-D(t)$ and the system starts empty, implies that 
...
\end{pyverbatim} \\this gives ??PythonTeX?? in pdf file

Another interesting queueing rule is to sort in increasing job size;
\begin{pyverbatim} \\this gives ??PythonTeX?? in pdf file
