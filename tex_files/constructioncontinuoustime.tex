...
\begin{exercise}\label{ex:25}
 Assume that $X_k = 10$ minutes and $S_k = 11$ minutes for all
    ...
    \begin{hint}
      For $L(A_k-)$ compute first $A(t)$ and $D(t)$. 
    \end{hint}
  \begin{solution}
 $\lambda=6$ per hour, and $\mu=60/11$ per hour. Note that
      ...
      \begin{equation*}
      L(A_k-) = k-1 - D(A_k-) = k- 1 - D(10k-) = k- 1 - \lfloor \frac{10(k-1)}{11} \rfloor.
      \end{equation*}
      The computation is a bit tricky since sometimes  arrivals  and  departures coincide.
