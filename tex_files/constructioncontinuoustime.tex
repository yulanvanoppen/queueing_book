...
\begin{exercise}[\faFlask]\label{ex:22}
...
Observe in Figure~\ref{fig:waitingtimegg1} that the waiting time of the $k$th arrival must be equal to the waiting time of the $k-1$th customer plus the amount of \recall{service time} required by job $k-1$ minus the time that elapses between the arrival of job $k-1$ and job $k$, unless the server becomes idle between jobs $k-1$ and $k$.
In other words,
...
\end{tikzpicture}
\caption{Construction of the single-server queue in continuous time. The
  sojourn time~$W_{k}$ of the~$k$th job is the sum of the work in queue~$W_{Q,k}$ at its arrival epoch~$A_k$ and its service time~$S_k$;
...
The \recall{sojourn time}, or \emph{waiting time in the system}, $W_k$, is the
...
\begin{exercise}[\faFlask]\label{ex:25}
...
\begin{exercise} [\faFlask] Why do the recursions of Eq.~(\ref{eq:45}) work? 
  \begin{solution}
...
\begin{exercise}[\faCalculator] It is clear  that $L(t) = A(t)-D(t)$. Show that, when the system starts empty, this implies that
...
\begin{exercise}[\faFlask] Show  that $L(A_k)>0 \implies A_k \leq D_{k-1}$.
...
For the next exercise we need an important definition.
Write $f(x+) = \lim_{h\downarrow0} f(x+h)$ for the right limit of a function $f$ at $x$ and $f(x-) = \lim_{h\downarrow0} f(x-h)$ for the left limit.
Note that $f$ is continuous at $x$ when $f(x-)=f(x+)$.
...
\begin{exercise}[\faRocket\faRocket] Try to extend the recursions~(\ref{eq:45})  to a situation in which one queue is
