...
\begin{question}  Why do the recursions Eq.~(\ref{eq:45}) work? 
  \begin{solution}
...
\begin{question} Show  that $L(A_k)>0 \implies A_k \leq D_{k-1}$.
...
\begin{question}  Try to extend the recursions~(\ref{eq:45})  to a situation in which one queue is
...
\begin{question} Suppose one server serves two
