...
$\{A(t)\}$ we can find $\{X_k\}$. In Figure~\ref{fig:waitingtimegg1}
...
becomes idle between jobs $k-1$ and $k$, see Figure.
In other words,
...
\draw[dotted] (1,0) -- (Wk) node[midway,fill=white,rotate=90] {$W_{Q,k}$};
...
\draw[|-|]
node[left] (c1) at (1,-0.5) {$A_k$}
node[right] (c2) at (6,-0.5) {$D_k$}
(c1) -- (c2)
node[midway, fill=white] {$W_k$};

\draw[dotted] (Wk) -- (Sk) node[midway,fill=white,rotate=90] {$S_k$};
\draw[dashed] (Ak1) -- (6,0); % node[midway,fill=white,rotate=90] {$S_k$};

\draw[-] (Sk) to (Ak1);
\node (Sk1) at (5,3) [closed, label={}];
\draw[dotted] (Ak1) -- (Sk1) node[midway,fill=white,rotate=90] {$S_{k+1}$};
\draw[dotted] (5,0) -- (Ak1);
\draw (Sk1) -- (8,0);

\draw[|-|]
node[left] (c1) at (5,-1) {$A_{k+1}$}
node[right] (c2) at (8,-1) {$D_{k+1}$}
(c1) -- (c2)
...
(c1) -- (c2)
node[midway, fill=white, rotate=90] {$S_{k+2}$};

\draw[|-|]
node[left] (c1) at (9,-1.5) {$A_{k+2}$}
node[right] (c2) at (11,-1.5) {$D_{k+2}$}
(c1) -- (c2)
node[midway, fill=white] {$W_{k+2}$};

\draw[|-|]
node[left] (c1) at (1,-1.5) {$A_{k}$}
node[right] (c2) at (5,-1.5) {$A_{k+1}$}
(c1) -- (c2)
node[midway, fill=white] {$X_{k+1}$};
\end{tikzpicture}
\caption{Construction of the $G/G/1$ queue in continuous time. The
  ...
  $W_{Qk,k}$ at its arrival epoch $A_k$ and the its service time
  $S_k$; its depature time is then $D_k=A_k + W_k$. The waiting time
  of job $k+1$ is clearly equal to $W_k-X_{k+1}$. We also see that job
  $k+2$ arrives at an empty system, hence its sojourn time
  $W_{k+2}=S_{k+2}$. The lines with slope $-1$ show the behavior the
...
points $W_k$ and have slope -1, unless the line hits the $x$-axis, in
which case the virtual waiting time remains zero until the next
arrival occurs.  Thus, the lines with slope $-1$ in
Figure~\ref{fig:waitingtimegg1} shown an example of (a sample path of)
...
\begin{question}
  ...
  previous exercise.
    \begin{solution}
You can check the files on github, in the progs directory. 
\begin{itemize}
\item \texttt{mm1.py} is the python implementation
\item \texttt{mm1.R} is the julia implementation
