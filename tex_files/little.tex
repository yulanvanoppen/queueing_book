...
\begin{exercise}[\faFlask]\label{ex:42}
...
\begin{exercise}[\faBalanceScale]\label{ex:37}
  %As a useful first application, consider the server of a $G/G/1$ queue as a system by itself.
By applying Little's law in a suitable way we can prove for the $G/G/1$ queue that $\E{L_S} = \rho = \lambda \E S$.
\begin{solution}
  Assume the system is rate-stable, for otherwise all relevant limits do not exist.
...
\begin{exercise}[\faFlask]\label{ex:43}
...
\begin{exercise}[\faBalanceScale]\label{ex:44}
