...
expectation need not be the same as~\eqref{eq:EQ}, c.f.  the
next exercise.  Hence, in general:
  \begin{equation*}
...
\begin{question}
Design a queueing system in which the load as seen by the server is very
...
\end{equation*}
where, by \eqref{eq:Lk}, $L_k=L(A_k)$ is the number in the system at the arrival epoch of the $k$th job. 


  \begin{hint}
    
  \hint{Consider a queueing system with constant service and
    interarrival times.}
  \end{hint}
\begin{solution}
...
\hint{Why is \eqref{eq:Qm} not the same as the number of
