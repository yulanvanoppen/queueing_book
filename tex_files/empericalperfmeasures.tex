\section{(Limits of) Emperical Performance Measures}
...
therefore define the \recall{average waiting time} as
...
measures.  Note also that these measures are \recall{averages as
  observed by arriving jobs}: the first job has a waiting time $W_1$
...
waiting time as `seen by arrivals'.  The \recall{distribution of the
  waiting times at arrival times} can  be found by counting:
...
\recall{distribution of $\{L(t)\}$ as seen by customers upon arrival},
is
...
the \recall{time-average fraction of time the system contains at most $m$
jobs}:
\begin{equation}
  \label{eq:47}
...
edge[pil,bend left=25, <-] (mu.east);
    ...
    \node[block, right=1cm of  D_t] (gamma) {$\frac{D(t)}t \to \gamma$} 
    edge[pil, <-]  (D_t);
    ...
    \node[block, right=1cm of  Q_t] (hoi) {$\gamma \leq \lambda$} 
    edge[pil, <-]  (gamma)
...
A final important topic is the, so called, limiting behavior of a
...
is known as the \recall{steady-state limit} of the sequence of random
variables $\{W_k\}$, and the distribution of~$W$ is known as the
\recall{limiting} or \emph{stationary distribution} of $\{W_k\}$.
