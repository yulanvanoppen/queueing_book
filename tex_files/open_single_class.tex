...
We chop up the problem in small steps.
  ...
  \item What is the density of $D_{n} - D_{n-1}$ when job $n$ sees an
    empty system?
  \item What is $\P{B}$, i.e., the probability that a job arrives at a
    busy station?
  ...
  \hint{ The final result is
    $f_D(t) = f_{X+S}(t) \P{I} + f_S \P B = (1-\rho) f_{X+S}(t) +
    \rho \mu e^{-\mu t}$.
    Now use the above to simplify and see that
...
&=  \lambda e^{-\lambda t}.
        \end{split}
      \end{equation*}
    \item       If the server is busy when a job arrives,  the density of the time to the next departure epoch must be exponential; note that we use the memoryless
      property of the exponential distribution of the service times. Therefore, the density when the server is busy      is $\mu e^{-\mu t}$.
...
\begin{question}(Linear algebra refresher) Can you find an example to
...
\begin{question}(Linear algebra refresher, 2) Suppose the matrix $A$
  ...
  \begin{solution}
    ...
    into a new vector $Ax\in \R^3$. Thus, $A$ typically changes the
    size and direction of $x$. 
