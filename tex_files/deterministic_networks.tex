...
\begin{exercise}
Suppose we would release just one job in the system: only when the first job has visited all stations, the next job is released. What is the capacity of the network?
\begin{solution}
In this case, the capacity $\mu$ of the network is $1/T_0$: only one server at a time is serving a job, the rest of the servers are forced to be idle.
\end{solution}
\end{exercise}

\begin{exercise}
Motivate that if we are prepared to release $w$ jobs, the capacity of the network is
given by
\begin{equation*}
...
\end{cases}
\end{equation*}
is the waiting time in the network.
\begin{hint}
  Use Little's law.
\end{hint}
  \begin{solution}
...
Thus, to balance waiting times and capacity, we need to tune the amount of work in the system. The critical WIP $W_0$ is the minimal WIP we need to keep all bottleneck machines occupied without adding extra waiting time.
  \end{solution}
\end{exercise}
...
here. Warning, these problems are quite challenging, despite their apparent simplicity; I had much less trouble making the problems than solving them. In fact, to ensure that
