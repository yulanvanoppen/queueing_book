\section{Some Useful Identities}\label{sec:some-usef-ident}
...
\end{equation*}
is called the average \recall{remaining service time} as seen at
...
\begin{question}
  ...
  \begin{itemize}
  \item Calculate the state probabilities.
    \begin{solution}
      \TBD
    \end{solution}
  \end{itemize}
...
<<term=False,fig=True,caption="The waiting time distribution for various values of the arrival rate.">>=
import numpy as np
...
performance measures for the analysis of closed-queueing network.
  ...
  \hint{ Interpret each component at the right hand side of the equation
    and generalize it to a multi-server queueing system. Then use Exercise~\ref{ex:10}.}
  \begin{solution}
